...
exports.validateCreateSourceTask = (req, res, next) => {
    let createSourceTask = req.body;
    // checking if input was inserted
    if (!createSourceTask.project_title)
        return res.status(400).json({ msg: 'project title required', status: 'failed' });
    // checking if input was inserted
    if (!createSourceTask.thematic_area)
        return res.status(400).json({ msg: 'thematic area required', status: 'failed' });
...
exports.validateTeamSourceProject = (req, res, next) => {
    req.body.values = JSON.parse(req.body.values);
    let teamSourceProject = req.body.values;
    // checking if input was inserted
    if (!teamSourceProject.project_title)
        return res.status(400).json({ msg: 'project title is required', status: 'failed' });
    // checking if input was inserted
    if (!teamSourceProject.thematic_area)
        return res.status(400).json({ msg: 'thematic area required', status: 'failed' });
    // checking years_of_experience
...
exports.validateContractConsultant = (req, res, next) => {
    let createSourceTask = req.body;
    // checking if input was inserted
    if (!createSourceTask.project_title)
        return res.status(400).json({ msg: 'project title required', status: 'failed' });
